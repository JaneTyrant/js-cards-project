...
function createActorsCard(actor) {
    ...
    const actorsName = createElement('p', {classNames: ['actors-name'], attributes: {'id': id, 'data-id': id}}, actorName );
    ...
    const initialsWrapper = createElement('div', {classNames: ['initials-wrapper', 'color-secondary'], attributes: {'data-id': id}, name: [firstName, lastName]} );
    const photoWrapper = createElement('div', {classNames: ['photo-wrapper'], attributes: {'data-id': id}}, initialsWrapper, cardPhoto );
    const cardArticle = createElement('article', {classNames: ['card-article'], attributes: {'data-id': id}, events: {'click': handleArticle}}, photoWrapper, actorsName, ...socials );
    return cardArticle;
}
...
function handleArticle({ target }) {
  ...
  if (!actorsNames.includes(actorsName)) {
    ...
    const actorsNameItem = createElement("li", {classNames: ['actors-name']}, actorsName);
